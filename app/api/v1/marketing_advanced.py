"""Marketing improvements — campaigns, segmentation, automation."""
...
router = APIRouter(prefix="/marketing-advanced", tags=["Napredni marketing"])
...
def list_segments(db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    return {
        "segments": [
            ...
            {
                "id": 5, "name": "Vegetarijanci",
                "description": "Stranke z vegetarijanskimi naročili",
                "count": 180,
                "rules": [{"field": "dietary", "operator": "=", "value": "vegetarian"}],
                "last_updated": "2026-01-15",
            },
        ],
        "total": 5,
...
def get_customer_insights(db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    return {
        ...
        "segments": [
            {"name": "VIP", "count": 85, "revenue_share": 25.0, "avg_spend": 45.00},
            {"name": "Redni", "count": 320, "revenue_share": 40.0, "avg_spend": 35.00},
            {"name": "Občasni", "count": 505, "revenue_share": 25.0, "avg_spend": 28.00},
            {"name": "Novi", "count": 120, "revenue_share": 5.0, "avg_spend": 25.00},
            {"name": "Neaktivni", "count": 220, "revenue_share": 5.0, "avg_spend": 20.00},
        ],
        "behavior": {
            "avg_visits_per_month": 2.5,
            "avg_days_between_visits": 12.0,
            "peak_hours": ["12:00-13:00", "18:00-19:00"],
            "popular_days": ["Sobota", "Nedelja"],
        },
        "preferences": {
            "top_categories": ["Glavne jedi", "Pijače", "Sladice"],
            "dietary": {"vegetarian": 15, "gluten_free": 8, "vegan": 5},
        },
    }
