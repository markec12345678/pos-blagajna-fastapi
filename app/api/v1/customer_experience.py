"""Customer experience — surveys, feedback, experience optimization."""
...
router = APIRouter(prefix="/customer-experience", tags=["Izkušnja strank"])
...
def respond_to_feedback(feedback_id: int, data: FeedbackResponse, db: Session = Depends(get_db), user=Depends(get_current_user)):
    """Odgovori na povratno informacijo."""
    return {
        "message": "Odgovor poslan",
        "feedback_id": feedback_id,
        "response": data.response,
        "responded_by": user.username if user else "Unknown",
