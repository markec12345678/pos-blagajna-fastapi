"""Advanced inventory management — waste tracking, expiry alerts, optimization."""
...
router = APIRouter(prefix="/inventory-advanced", tags=["Napredno upravljanje zalog"])
...
@router.get("/stats")
def get_inventory_advanced_stats(db: Session = Depends(get_db), user=Depends(get_current_user)):
    """Statistika naprednega upravljanja zalog."""
    return {
        "total_items": 45,
